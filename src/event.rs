...
pub fn update_code_inspector_for_commit(app: &mut App) {
    let Some(selected_index) = app.commit_list_state.selected() else {
        return;
    };

    if selected_index >= app.commit_list.len() {
        return;
    }
    ...
    app.selected_commit_hash = Some(commit_data.hash.clone());
...
fn extract_commit_data(app: &App, index: usize) -> CommitData {
    let commit = &app.commit_list[index];
    CommitData {
...
fn save_cursor_state(app: &App) -> CursorState {
    CursorState {
        old_cursor_line: app.cursor_line,
        old_commit_hash: app.selected_commit_hash.clone(),
        old_scroll_vertical: app.inspector_scroll_vertical,
        old_cursor_viewport_offset: app.cursor_line.saturating_sub(app.inspector_scroll_vertical as usize),
    }
}
...
fn restore_cursor_position(
    ...
    app.cursor_line = cursor_state.old_cursor_line;
...
fn handle_content_load_error(app: &mut App, error: Box<dyn std::error::Error>) {
    ...
    app.status_message = format!("Failed to load content: {}", error);
}
...
fn handle_no_file_context(app: &mut App, commit_data: &CommitData) {
    ...
    app.cursor_line = 0;
    app.last_commit_for_mapping = None;
    app.status_message = format!("Viewing commit: {}", commit_data.short_hash);
}
