...
impl LineMapping {
    ...
    pub fn find_nearest_mapped_line(&self, old_line: usize, search_radius: usize) -> Option<usize> {
        ...
        if let Some(mapped) = self.map_line(old_line) {
            debug!(
                "find_nearest_mapped_line: Found exact mapping for line {} -> {}",
                old_line, mapped
            );
            return Some(mapped);
        }
        ...
        debug!(
            "find_nearest_mapped_line: No mapping found within radius {} for line {}",
            search_radius, old_line
        );
        None
    }
    ...
    pub fn find_exact_content_match(
        ...
        debug!(
            "find_exact_content_match: Searching for line {} in file {:?} from {} to {}",
        ...
        if old_line >= old_lines.len() {
            debug!(
                "find_exact_content_match: old_line {} >= old_lines.len() {}, returning None",
                old_line,
                old_lines.len()
            );
            return Ok(None);
        ...
        debug!(
            "find_exact_content_match: Target line content: '{}'",
            target_line_content
        );
        ...
        debug!(
            "find_exact_content_match: Searching through {} new lines",
            new_lines.len()
        );
        ...
        let matches: Vec<usize> = new_lines
            ...
            .filter_map(|(idx, &line)| {
                if line == target_line_content {
                    debug!(
                        "find_exact_content_match: Found match at line {}: '{}'",
                        idx, line
                    );
                    Some(idx)
                } else {
        ...
        debug!(
            "find_exact_content_match: Found {} matches: {:?}",
            matches.len(),
            matches
        );
        ...
        match matches.len() {
            1 => {
                info!(
                    "find_exact_content_match: SUCCESS - Found unique match at line {}",
                    matches[0]
                );
                Ok(Some(matches[0]))
            }
    ...
    pub fn find_content_aware_nearest_mapped_line(
        ...
        debug!(
            "find_content_aware_nearest_mapped_line: Starting search for line {} with radius {}",
            old_line, search_radius
        );
        ...
        debug!(
            "find_content_aware_nearest_mapped_line: Looking for content: '{}'",
            target_content
        );
...
pub fn map_lines_between_commits(
    ...
    debug!(
        "map_lines_between_commits: Old content has {} lines, new content has {} lines",
        old_lines.len(),
        new_lines.len()
    );
...
fn get_file_content_at_commit(
    ...
    let tree = commit.tree().map_err(|e| LineMappingError::DiffFailed {
        reason: e.to_string(),
    })?;
    ...
    let blob = file_entry
        .object()
        .map_err(|e| LineMappingError::DiffFailed {
            reason: e.to_string(),
        })?
        .try_into_blob()
...
mod tests {
    ...
    fn test_same_commit_mapping() {
        ...
        let mapping =
            map_lines_between_commits(&repo, &commit_hash, &commit_hash, Path::new("test.txt"))
                .unwrap();
    ...
    fn test_simple_line_addition() {
        ...
        let mapping =
            map_lines_between_commits(&repo, &commit1, &commit2, Path::new("test.txt")).unwrap();
    ...
    fn test_empty_file_handling() {
        ...
        let mapping =
            map_lines_between_commits(&repo, &commit1, &commit2, Path::new("empty.txt")).unwrap();
    ...
    fn test_exact_content_fallback_single_match() {
        ...
        let result =
            mapping.find_exact_content_match(1, &repo, &commit1, &commit2, Path::new("test.txt"));
    ...
    fn test_exact_content_fallback_integration() {
        ...
        let mapping =
            map_lines_between_commits(&repo, &commit1, &commit2, Path::new("code.js")).unwrap();

        // Line 3 "function bar() {" should map to line 0 via exact content match
        let result =
            mapping.find_exact_content_match(3, &repo, &commit1, &commit2, Path::new("code.js"));
        assert_eq!(result.unwrap(), Some(0));

        // Line 4 "  return 24;" should map to line 1 via exact content match
        let result =
            mapping.find_exact_content_match(4, &repo, &commit1, &commit2, Path::new("code.js"));
    ...
    fn test_find_nearest_with_exact_content_fallback_integration() {
        ...
        match result {
            Ok(Some(new_line)) => {
                ...
                let old_content =
                    get_file_content_at_commit(&repo, &commit1, Path::new("test.txt")).unwrap();
                let new_content =
                    get_file_content_at_commit(&repo, &commit2, Path::new("test.txt")).unwrap();
                let old_lines: Vec<&str> = old_content.lines().collect();
                let new_lines: Vec<&str> = new_content.lines().collect();
    ...
    fn test_content_aware_vs_standard_nearest_neighbor() {
        ...
        let mapping =
            map_lines_between_commits(&repo, &commit1, &commit2, Path::new("code.txt")).unwrap();
    ...
    fn test_content_aware_exact_mapping_verification() {
        ...
        let mapping =
            map_lines_between_commits(&repo, &commit1, &commit2, Path::new("struct.rs")).unwrap();
    ...
    fn test_end_to_end_content_aware_fallback_integration() {
        ...
        let mapping =
            map_lines_between_commits(&repo, &commit1, &commit2, Path::new("simple.rs")).unwrap();
        ...
        if let Some(result) = small_radius_result.unwrap() {
            ...
            let old_content =
                get_file_content_at_commit(&repo, &commit1, Path::new("simple.rs")).unwrap();
            let new_content =
                get_file_content_at_commit(&repo, &commit2, Path::new("simple.rs")).unwrap();
            let old_lines: Vec<&str> = old_content.lines().collect();
            let new_lines: Vec<&str> = new_content.lines().collect();
        ...
        let exact_result =
            mapping.find_exact_content_match(1, &repo, &commit1, &commit2, Path::new("simple.rs"));
