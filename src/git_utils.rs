...
pub fn get_commit_history_for_file(
    ...
    for commit_info in commit_iter {
        ...
        let modified_file = if commit.parent_ids().count() == 0 {
            ...
            let mut buf = Vec::new();
            tree.lookup_entry_by_path(normalized_path, &mut buf)?.is_some()
        } else {
            ...
            for parent_id in commit.parent_ids() {
                ...
                let current_entry = current_tree.lookup_entry_by_path(normalized_path, &mut current_buf)?;
                let parent_entry = parent_tree.lookup_entry_by_path(normalized_path, &mut parent_buf)?;
...
fn get_file_content_with_gix(
    ...
    let mut buf = Vec::new();
    let file_entry = tree.lookup_entry_by_path(file_path, &mut buf)?
...
mod tests {
    ...
    fn test_commit_history_only_shows_modifications() {
        ...
        let readme_commits = get_commit_history_for_file(&repo, "README.md").expect("Should get README.md history");
        let main_commits = get_commit_history_for_file(&repo, "src/main.rs").expect("Should get main.rs history");
        ...
        assert!(readme_commits.len() < main_commits.len(), 
        ...
        assert!(readme_commits.len() <= 10, 
            "README.md should not have more than 10 commits, got {}", readme_commits.len());
    ...
    fn test_commit_history_for_nonexistent_file() {
        ...
        let repo = open_repository(".").expect("Should be able to open repository");
        let commits = get_commit_history_for_file(&repo, "nonexistent/file.txt").expect("Should handle nonexistent file");
        
        assert!(commits.is_empty(), "Should have no commits for nonexistent file");
    }
